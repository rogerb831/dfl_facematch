...
ECHO.
ECHO. This is the data_dst directory version.
ECHO.
ECHO.
...
rem set fullPath=%WORKSPACE%\data_dst\aligned\%referenceFile%

pushd .
cd %WORKSPACE%\data_dst\aligned
set data_dstAlignedDir=%CD%
set fullPath=%data_dstAlignedDir%\%referenceFile%
popd
...
if NOT exist %fullPath% (
	...
	"%PYTHON_EXECUTABLE%" "%DFL_ROOT%"\facematch.py %fullPath% %data_dstAlignedDir%
	echo.
)
